module.exports = function(RED) {
  function RandomItemNode(config) {
    ...
    node.on("input", function(msg) {
      ...
      if (!Array.isArray(array)) {
        node.error(`${JSON.stringify(array)} is not a valid array`, msg);
        node.status({
          fill: "red",
          shape: "ring",
          text: "invalid array"
      ...
      const payload = randomItems.length === 1 ? randomItems[0] : randomItems;
      ...
      switch (config.outputType) {
        ...
        case "global":
          node
            .context()
            [config.outputType].set(contextKey.key, payload, contextKey.store);
          break;
        case "msg":
        default:
  ...
  function getArray(location, property, message) {
    ...
    return message.context()[location].get(contextKey.key, contextKey.store);
  }
